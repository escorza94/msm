<div class="max-w-4xl mx-auto mt-8">
    <div class="flex justify-between items-center mb-6 bg-white p-5 rounded-xl shadow-sm border border-gray-100">
        <div>
            <h2 class="text-2xl font-bold text-gray-800 flex items-center"><i class="fas fa-edit text-teal-500 mr-2"></i> Editar Artículo</h2>
            <p class="text-sm text-gray-500 mt-1">Actualiza el contenido del artículo de la Base de Conocimiento</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden p-6">
        <form action="<?= base_url('help/editar') ?>" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="titulo" value="<?= htmlspecialchars($articulo['titulo']) ?>" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-teal-500 outline-none text-sm transition">
                </div>
                ...
                <div>
                    ...
                    <select name="categoria_id" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-teal-500 outline-none text-sm bg-white">
                        ...
                        <?php foreach($categorias as $c): ?>
                            <option value="<?= $c['id'] ?>" <?= $c['id'] == $articulo['categoria_id'] ? 'selected' : '' ?>><?= htmlspecialchars($c['nombre']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="tipo" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-teal-500 outline-none text-sm bg-white">
                        <option value="publico" <?= $articulo['tipo'] == 'publico' ? 'selected' : '' ?>>Público (La IA lo usará para contestar a clientes)</option>
                        <option value="interno" <?= $articulo['tipo'] == 'interno' ? 'selected' : '' ?>>Interno (Solo visible para el Staff en el CRM)</option>
                    </select>
                </div>

                <div class="md:col-span-2">
                    <label class="block text-sm font-bold text-gray-700 mb-1">Contenido / Respuesta <span class="text-red-500">*</span></label>
                    <textarea name="contenido" required rows="8" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-teal-500 outline-none text-sm transition resize-none"><?= htmlspecialchars($articulo['contenido']) ?></textarea>
                </div>
            </div>
            <div class="pt-4 mt-6 border-t border-gray-100 flex justify-end gap-3"><a href="<?= base_url('help') ?>" class="px-5 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 font-medium transition text-sm">Cancelar</a><button type="submit" class="px-5 py-2.5 bg-teal-500 text-white rounded-lg hover:bg-teal-600 font-bold transition shadow-md text-sm">Actualizar Cambios</button></div>
        </form>
    </div>
</div>