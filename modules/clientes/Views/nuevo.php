<div class="max-w-3xl mx-auto mt-8">
    <div class="flex justify-between items-center mb-6 bg-white p-5 rounded-xl shadow-sm border border-gray-100">
        <div>
            <h2 class="text-2xl font-bold text-gray-800 flex items-center"><i class="fas fa-user-plus text-blue-500 mr-2"></i> Nuevo Cliente</h2>
            <p class="text-sm text-gray-500 mt-1">Registra un nuevo contacto, prospecto o comprador</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden p-6">
        <form action="<?= base_url('clientes/nuevo') ?>" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="nombre" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm transition shadow-sm" placeholder="Ej. Juan Pérez / Empresa S.A.">
                </div>
                
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Teléfono Móvil</label>
                    <input type="text" name="telefono" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm transition shadow-sm" placeholder="Ej. 555-123-4567">
                </div>
                
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Correo Electrónico</label>
                    <input type="email" name="correo" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm transition shadow-sm" placeholder="cliente@correo.com">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">RFC (Para Facturación)</label>
                    <input type="text" name="rfc" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm transition shadow-sm uppercase" placeholder="XAXX010101000">
                </div>
                ...
                <div>
                    ...
                    <select name="whatsapp_id" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none text-sm transition shadow-sm bg-green-50">
                        ...
                        <?php foreach($contactos_wa ?? [] as $wa): ?>
                            <option value="<?= htmlspecialchars($wa['whatsapp_id']) ?>"><?= htmlspecialchars($wa['nombre']) ?> (<?= explode('@', $wa['whatsapp_id'])[0] ?>)</option>
                        <?php endforeach; ?>
                    </select>
                    <p class="text-[10px] text-gray-400 mt-1">Sincroniza a este cliente con su ventana de chat en el CRM.</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2 relative">
                    ...
                    <textarea id="direccion_input" name="direccion" rows="3" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm transition shadow-sm resize-none" placeholder="Calle, Número, Colonia, Ciudad, Estado, CP..."></textarea>
                    
                    <!-- Campos ocultos para las coordenadas y enlace -->
                    <input type="hidden" name="latitud">
                    <input type="hidden" name="longitud">
                    <input type="hidden" name="enlace_maps">
                </div>
            </div>
            ...
            <div class="pt-4 mt-6 border-t border-gray-100 flex justify-end gap-3">
                ...
                <button type="submit" class="px-5 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-bold transition shadow-md flex items-center text-sm"><i class="fas fa-save mr-2"></i> Guardar Cliente</button>
            </div>
        </form>
    </div>
</div>
...
function initMapModal() {
    ...
    // Coordenadas por defecto (Centro de México o lo que esté en los inputs)
    ...
    updateMarkerPosition(marker.getPosition()); // Leer inicial
    ...
    // Integrar el buscador dentro del mapa
    ...
    // Intentar centrar el mapa si el usuario ya había escrito una dirección pero no tenía coordenadas
    ...
    if(!document.querySelector('input[name="latitud"]').value && currentAddress) {
        ...
        obtenerMiUbicacion(true); // Obtener ubicación automáticamente sin lanzar alertas de error
    }
}
