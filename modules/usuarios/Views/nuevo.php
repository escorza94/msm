<div class="max-w-3xl mx-auto mt-8">
    <div class="flex justify-between items-center mb-6 bg-white p-5 rounded-xl shadow-sm border border-gray-100">
        <div>
            <h2 class="text-2xl font-bold text-gray-800 flex items-center"><i class="fas fa-user-plus text-indigo-500 mr-2"></i> Nuevo Usuario</h2>
            <p class="text-sm text-gray-500 mt-1">Registra un nuevo miembro del equipo (Asesor o Administrador)</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden p-6">
        <form action="<?= base_url('usuarios/nuevo') ?>" method="POST" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nombre Completo <span class="text-red-500">*</span></label>
                    <input type="text" name="name" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-indigo-500 outline-none text-sm transition shadow-sm" placeholder="Ej. Carlos Martínez">
                </div>
                
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Correo Electrónico <span class="text-red-500">*</span></label>
                    <input type="email" name="email" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-indigo-500 outline-none text-sm transition shadow-sm" placeholder="carlos@muebleria.com">
                </div>
                
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Contraseña de Acceso <span class="text-red-500">*</span></label>
                    <input type="password" name="password" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-indigo-500 outline-none text-sm transition shadow-sm">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="whatsapp_id" id="whatsapp_id" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-indigo-500 outline-none text-sm transition shadow-sm font-mono">
                        ...
                        <?php foreach($contactos_wa ?? [] as $wa): ?>
                            <option value="<?= htmlspecialchars($wa['whatsapp_id']) ?>"><?= htmlspecialchars($wa['nombre']) ?> (<?= htmlspecialchars($wa['whatsapp_id']) ?>)</option>
                        <?php endforeach; ?>
                    </select>
            ...
            <div class="pt-4 mt-6 border-t border-gray-100 flex justify-end gap-3">
                <button type="submit" class="px-5 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-bold transition shadow-md flex items-center text-sm"><i class="fas fa-save mr-2"></i> Guardar Usuario</button>
            </div>
        </form>
    </div>
</div>
