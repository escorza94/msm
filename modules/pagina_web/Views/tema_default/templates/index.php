...
<body class="bg-fondo font-sans antialiased">
    ...
    <main>
        ...
        <?php foreach ($secciones as $seccion): ?>
                ...
                <section id="texto-<?= $seccion['id'] ?>" class="py-10 md:py-16 bg-fondo">
                    <div class="container mx-auto px-4 md:px-6 max-w-4xl text-gray-700">
                        ...
                        <div class="prose max-w-none text-center md:text-left leading-relaxed">
                            <?= nl2br($seccion['config']['contenido'] ?? '') ?>
                        </div>
                    </div>
                </section>
            ...
            <?php elseif ($seccion['tipo'] === 'grid_promociones'): ?>
                <!-- WIDGET: Grid de Promociones -->
                <?php $promociones_grid = $seccion['promociones'] ?? []; ?>
                <section id="promociones-<?= $seccion['id'] ?>" class="py-10 md:py-16 bg-fondo">
                    <div class="container mx-auto px-4 md:px-6">
                        <h2 class="text-2xl md:text-3xl font-bold text-center text-primario mb-2"><?= htmlspecialchars($seccion['config']['titulo_seccion'] ?? 'Promociones Calientes') ?></h2>
                        <?php if (!empty($seccion['config']['subtitulo'])): ?>
                            <p class="text-center text-gray-500 mb-8 md:mb-10 text-sm md:text-base"><?= htmlspecialchars($seccion['config']['subtitulo']) ?></p>
                        <?php endif; ?>

                        <?php if (empty($promociones_grid)): ?>
                            <div class="text-center text-gray-500 bg-gray-50 p-8 rounded-xl border border-gray-100 max-w-2xl mx-auto">
                                <i class="fas fa-ticket-alt text-4xl mb-3 opacity-30"></i>
                                <p class="font-medium text-gray-600">No hay promociones activas en esta colección.</p>
                            </div>
                        <?php else: ?>
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                            <?php foreach($promociones_grid as $promo): ?>
                            <div class="bg-gradient-to-br from-primario to-indigo-700 rounded-2xl shadow-xl text-white relative overflow-hidden flex flex-col transform hover:-translate-y-1 transition-transform duration-300 border border-white/10">
                                <!-- Recortes laterales tipo ticket -->
                                <div class="absolute top-1/2 -left-3 w-6 h-6 bg-fondo rounded-full transform -translate-y-1/2 shadow-inner z-20 hidden sm:block"></div>
                                <div class="absolute top-1/2 -right-3 w-6 h-6 bg-fondo rounded-full transform -translate-y-1/2 shadow-inner z-20 hidden sm:block"></div>
                                ...
                                <div class="p-6 flex-1 flex flex-col justify-between bg-black/10 relative z-10">
                                    <div class="mb-5">
                                        <?php if(!empty($promo['productos_incluidos'])): ?>
                                            ...
                                            <div class="space-y-2.5 mb-4">
                                                <?php foreach($promo['productos_incluidos'] as $prod): ?>
                                                    <div class="flex items-center gap-3 bg-white/10 rounded-lg p-2 hover:bg-white/20 transition">
                                        ...
                                        <?php endif; ?>
                                        <ul class="text-xs space-y-2 opacity-90 mb-2">
                                            <?php if($promo['monto_minimo'] > 0): ?><li><i class="fas fa-info-circle text-yellow-300 mr-1.5"></i> Compra mínima requerida: $<?= number_format($promo['monto_minimo'], 2) ?></li><?php endif; ?>
                                            <?php if($promo['cantidad_minima'] > 0): ?><li><i class="fas fa-info-circle text-yellow-300 mr-1.5"></i> Debes llevar al menos <?= $promo['cantidad_minima'] ?> piezas</li><?php endif; ?>
                                        </ul>
                                    </div>
                                    <a href="https://wa.me/<?= htmlspecialchars($config['whatsapp_numero'] ?? '') ?>?text=Hola,%20me%20interesa%20la%20promoción:%20<?= urlencode($promo['nombre']) ?>" target="_blank" class="block w-full bg-white text-primario text-center py-3 rounded-lg font-bold hover:bg-gray-50 transition shadow-md hover:shadow-lg uppercase text-sm tracking-wide">Reclamar Cupón <i class="fas fa-arrow-right ml-1"></i></a>
                                </div>
                            </div>
                            <?php endforeach; ?>
                        </div>
