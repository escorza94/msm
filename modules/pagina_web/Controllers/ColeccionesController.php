...
class ColeccionesController extends Controller {
    ...
    public function index() {
        ...
        try {
            ...
            $colecciones = $db->query("
                SELECT c.id, c.nombre, c.slug, c.estado, c.descripcion, COUNT(i.id) as total_productos_visibles
                ...
                GROUP BY c.id, c.nombre, c.slug, c.estado, c.descripcion
                ORDER BY c.id DESC
            ")->fetchAll();
    ...
    public function postEditar() {
        ...
        try {
            $db->beginTransaction();
            $db->prepare("UPDATE tienda_colecciones SET nombre=?, slug=?, descripcion=? WHERE id=?")->execute([$nombre, $slug, $descripcion, $id]);
